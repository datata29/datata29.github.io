...
* 출력 형식 지정 함수: 데이터 타입이나 출력 형태를 지정하는 함수 <br>
  - 대표 함수 : FORMAT_DATETIME , PARSE_DATETIME , DATETIME_TRUNC ,EXTRACT <br><br>

* 계산 함수 : 날짜를 계산하는 함수 <br>
  -  대표 함수 : DATETIME_ADD , DATETIME_SUB , DATETIME_DIFF <br><br>

각 대표 함수에 대한 설명은 아래 표에서 살펴 보자. <br><br>

<img src="{{ site.baseurl }}/assets/img/sql17_date_function.png" width="70%" height="70%"> <br>

특히 출력 형식 지정함수에서 FORMAT_DATETIME과 PARSE_DATETIME을 헷갈려하는 경우가 많다.<br>
( ex. 20240825 -> 2024-08-25 )<br>

FORMAT_DATETIME은 datetime을 string으로, PARSE_DATETIME은 string을 datetime으로 바꿔서 출력한다.<br><br>

특히 PARSE_DATETIME은 날짜값인데 '%Y-%m-%d 형식이 아닌 string값을 날짜 타입으로 변경할때 유용하다. <br>
( ex. 20240825 -> 2024-08-25 ) <br><br>
...
구문이 길어지기 때문에  PARSE_DATETIME( ) 을 사용하는 것을 권장한다.	<br><br>
...
ㄴ _table_suffix : 20240825 -> yyyymmdd : 2024-08-25	<br><br>

## 3. Appendix

- 다루고자 하는 날짜 타입 (Date, Datetime ,Timestamp )에 따라 함수명을 변경할 수 있다.<br>
...
참고로 DATE와 DATETIME, TIMESTAMP는 표현할 수 있는 날짜 범위에 차이가 있다.<br><br>
...
- TIMESTAMP : 날짜를 마이크로초단위 ( 초 이하 소숫점 6자리 ) 로  정밀하게 저장하고 있는 날짜 타입 <br> 	
  빅쿼리상에서는 표기시 타임존이 같이 출력된다. ( ex: 2024-01-01 12:30:12.000000 UTC )	<br>
...
- 날짜 출력 형식은 아래 표현을 참고하면 된다. <br><Br>
