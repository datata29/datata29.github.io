...
이 차트를 보고 트래픽이 견조한지, 운영상 이슈는 없었는지 , 앞으로의 
과제는 무엇이며 어떻게 개선할 수 있을지 종합적으로 살펴보자. <br>	
( 샘플 데이터를 이용하였습니다 )	<br>
...
A1.
  ...
  이동평균을 볼때 DAU를 보조 지표로 같이 봐야겠다. <br>
...
```
STL(Seasonal-Trend decomposition using LOESS) decomposition은 시계열 데이터를								
관측치 = 추세(Trend) + 시즌(Seasonal) + 잔차(Residual) 로 분해하는 방법.

주간(요일)처럼 강한 주기 패턴이 있을 때, 주기성을 걷어내고도 남는 설명되지 않는 충격(Residual)을 분리해 이상치를 더 공정하게 잡는 데 유용.								
...
5. 권장 액션	<br>							
	...
	- 이상치 대응 (ex. 11/15)		<br>					
		- STL residual 기반 자동 알람 룰(예: |z|≥3) + 장애 지표(로그인 실패/에러율/결제 실패) 연동 <br>						
